def distance(first, second):
    #my_matrix = generate_matrix(10,10)
    ...
    final_first = move_through_matrix(first, current_first)
    print("Final first = ", final_first)
    final_second = move_through_matrix(second, current_second)
    print("Final second = ",final_second)
...
def move_through_matrix(arr_steps, point_to_move):
    ...
    for instruction in arr_steps:
        ...
        if horizontal_steps > 0:
            for a in range(0, horizontal_steps):

                if vertical_steps == 0:
                    print("Passing through ",  [a, 0])
                else:
                    for b in range(0, vertical_steps):
                        print("Passing through ",  [a, b])


        # point_to_move[0] += 
        #Move vertically
        # point_to_move[1] += 
        print("position = ", point_to_move)
        points_it_has_passed.append(point_to_move.copy())
        print("total = ", points_it_has_passed)
    
    print("Result = ", point_to_move)
    return points_it_has_passed



def generate_matrix(horizontal, vertical):
    matrix = []
